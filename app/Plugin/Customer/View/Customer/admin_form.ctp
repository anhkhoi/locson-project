...
$this->Html
        ...
        ->addCrumb(__('Customers'), array('controller' => 'customer', 'action' => 'index'));

if ($this->request->params['action'] == 'admin_edit') {
    $this->Html->addCrumb($this->request->data['Customer']['title']);
}
...
<div class="row-fluid">
    <div class="span8">
        ...
        <ul class="nav nav-tabs">
            <li><a href="#contact-basic" data-toggle="tab"><?php echo __('Customer'); ?></a></li>
...
<script type="text/javascript">
    $(function() {
        if ($("#CustomerImages").val() !== '') {
            $('<img src="' + $("#CustomerImages").val() + '" />').appendTo($('#divfeature'));
        }

        $("#btnUpload").click(function() {
            window.open('<?php echo 'http://' . $_SERVER['HTTP_HOST']; ?>/admin/file_manager/attachments/browse?type=feature&inputHidden=CustomerImages&domContain=divfeature', '', 'width=900,height=600');
        });
    });
</script>