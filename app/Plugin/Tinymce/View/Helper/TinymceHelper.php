...
EOF;
    ...
    public function selectURL() {
        //Add image feature for form
        
        
        $domFeature = @$_SESSION['AttachImage']['domContain'];
        $inputHidden = @$_SESSION['AttachImage']['inputHidden'];
        $listImage = @$_SESSION['AttachImage']['listImage'];
        
        $output = <<<EOF
        function selectURL(url, title, description,id) {
                if('$domFeature' !== ''){
                    if('$inputHidden' !== ''){
                        opener.document.getElementById('$inputHidden').value = '/uploads/'+url;
                    }
                    opener.document.getElementById('$domFeature').innerHTML = '<img src="/uploads/'+url+'" />';
                    close();
                }
                if('$listImage' !== ''){
                    var data = opener.document.getElementById('$listImage').value;
                    var strId = data.concat(id + ',');
                    opener.document.getElementById('$listImage').value = strId;
                    close();
                }
